...
def main():
    ...
    optimizer = optim_lookahead.Lookahead(optimizer)
    ...
    for epoch in range(epochs):
        ...
        train_loss = 0.0
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1} - Training", disable=(local_rank != 0)):
            optimizer.zero_grad()
        ...
        val_loss = 0.0
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"Epoch {epoch+1} - Evaluating", disable=(local_rank != 0)):
                input_ids = batch['input_ids'].to(device)
                attention_mask = batch['attention_mask'].to(device)
                start_positions = batch['start_positions'].to(device)
                end_positions = batch['end_positions'].to(device)
